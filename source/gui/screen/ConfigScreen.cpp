...
ConfigScreen::ConfigScreen() : oldStorage(Configuration::getInstance().storageSize())
{
    ...
    tabButtons[1].push_back(new Button(122, 86, 15, 12, [](){ Gui::setNextKeyboardFunc([](){ inputText([](std::string config){ Configuration::getInstance().defaultOT(config); }, Configuration::getInstance().defaultOT()); }); return false; }, ui_sheet_button_info_detail_editor_light_idx, "", 0.0f, 0));
    ...
    tabButtons[1].push_back(new Button(122, 206, 15, 12, [](){ Gui::setNextKeyboardFunc([](){ inputText([](std::string config){ Configuration::getInstance().cloudLoginCode(config); }, Configuration::getInstance().cloudLoginCode()); }); return false; }, ui_sheet_button_info_detail_editor_light_idx, "", 0.0f, 0));
...
void ConfigScreen::draw() const
{
    ...
    else if (currentTab == 1)
    {
        ...
        Gui::dynamicText(Configuration::getInstance().cloudLoginCode(), 150, 204, FONT_SIZE_14, FONT_SIZE_14, COLOR_WHITE, TextPosX::LEFT, TextPosY::TOP);
